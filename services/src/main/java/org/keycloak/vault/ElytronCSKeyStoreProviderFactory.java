 ...
 * <ul>
 *     <li>location (required): the path to he keystore file that contains the secrets. This file is created and managed by Elytron
 *     using either the {@code elytron} subsystem in WildFly/EAP or the {@code elytron-tool.sh} script</li>
 *     <li>secret (required): the keystore master secret. Can be specified in clear text form or in masked form. The masked form
 *     can be generated using the {@code elytron-tool.sh} script. For further details, check the Elytron tool documentation.</li>
 *     <li>type (optional): the keystore type. Defaults to {@code JCEKS}.</li>
 * </ul>
 * <p/>
 * If any of the required configuration attributes is missing, the factory logs a WARN message indicating that it has not
 ...
 * a {@link VaultNotFoundException} is raised.
 *
 * @author <a href="mailto:sguilhen@redhat.com">Stefan Guilhen</a>
 */
public class ElytronCSKeyStoreProviderFactory implements VaultProviderFactory {

    private static final Logger logger = Logger.getLogger(MethodHandles.lookup().lookupClass());

    public static final String PROVIDER_ID = "elytron-cs-keystore";

    private static final String CS_LOCATION = "location";
    private static final String CS_TYPE = "type";
    private static final String CS_SECRET = "secret";
    ...
    public VaultProvider create(KeycloakSession session) {
        if (this.credentialStoreLocation == null || this.credentialStoreSecret == null) {
            logger.debug("Cannot create an elytron-based vault provider since it's disabled or not initialized correctly");
            return null;
        }
        Map<String, String> attributes = new HashMap<>();
        attributes.put(CS_LOCATION, this.credentialStoreLocation);
        attributes.put("keyStoreType", this.credentialStoreType);

        return new ElytronCSKeyStoreProvider(getRealmName(session), attributes, new CredentialStore.CredentialSourceProtectionParameter(
    ...
    public void init(Config.Scope config) {
        ...
        if (this.credentialStoreLocation == null) {
            logger.warn("ElytronCSKeyStoreProviderFactory not properly configured - missing store location");
            return;
        }
        ...
        if (this.credentialStoreSecret == null) {
            logger.warn("ElytronCSKeyStoreProviderFactory not properly configured - missing store secret");
            return;
        }
        this.credentialStoreType = config.get(CS_TYPE, "JCEKS");
